...
function perkRows(): string {
  return PERKS.map((p) => {
    const button = p.href
      ? `\n    <a href="${SITE}${p.href}" style="display:inline-block;margin-top:10px;padding:7px 14px;font-size:10px;font-weight:700;letter-spacing:0.12em;color:#FF6B35;border:1px solid #FF6B35;text-decoration:none;text-transform:uppercase;">${p.cta} &rarr;</a>`
      : ''
  ...
  <td style="border-left:2px solid #FF6B35;padding:2px 0 2px 16px;">
    <p style="margin:0 0 4px 0;font-size:11px;font-weight:700;letter-spacing:0.14em;color:#F5F5F5;text-transform:uppercase;">${p.title}</p>
    <p style="margin:0;font-size:12px;line-height:1.7;color:#8A9AB5;">${p.desc}</p>${button}
  </td>
</tr></table>`
  }).join('\n  ')
}
...
function architectAvatars(): string {
  const cells = ARCHITECTS.map((a) => {
    ...
    <a href="${a.href}" style="text-decoration:none;">
      ${face}
      <p style="margin:8px 0 0 0;font-size:10px;letter-spacing:0.06em;color:#FF6B35;">@${a.handle}</p>
    </a>
  </td>`
  }).join('\n  ')
...
<tr><td style="background-color:#0D1020;border:1px solid #1E2840;padding:40px 36px;">
  <p style="margin:0 0 22px 0;font-size:9px;letter-spacing:0.3em;color:#4A5570;text-transform:uppercase;">// Voyager confirmed</p>
  <h1 style="margin:0 0 18px 0;font-size:22px;font-weight:700;letter-spacing:0.08em;color:#F5F5F5;line-height:1.3;">YOU ARE NOW<br/>A MULTIVERSE VOYAGER</h1>
  <p style="margin:0 0 18px 0;font-size:12px;line-height:1.9;color:#8A9AB5;">${greet}your Initial Voyager Pack is confirmed. Welcome aboard.</p>
  <p style="margin:0 0 26px 0;font-size:12px;line-height:1.9;color:#8A9AB5;">The physical items&mdash;the welcome letter, your metal Voyager badge, and the classified component parts&mdash;are currently in assembly. We will initiate shipment with the very first batch and notify you.</p>

  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 28px 0;"><tr><td align="center">
    <img src="${SITE}/voyager-pack/pack-email.png" width="210" alt="Initial Voyager Pack" style="display:block;width:210px;max-width:62%;height:auto;border:1px solid #1E2840;"/>
  </td></tr></table>

  <p style="margin:0 0 16px 0;font-size:9px;letter-spacing:0.25em;color:#4A5570;text-transform:uppercase;">// What you have unlocked</p>
  ${perkRows()}
  ...
  <p style="margin:0 0 18px 0;font-size:12px;line-height:1.9;color:#C7D2E6;">The Collective is built by the people inside it, and we would rather hear from you than at you. Reply directly to this transmission, or reach our Architects on X:</p>
  ${architectAvatars()}
