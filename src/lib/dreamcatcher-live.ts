...
export function groupDreamcatcherLiveVideos(assets: CosmoAsset[]): DreamcatcherLiveVideoLibrary {
  ...
  for (const asset of assets) {
    if (asset.media !== 'video' || !/^https?:\/\//.test(asset.url)) continue
...
export function syncDreamcatcherPlaybackToState(
  ...
  if (current.phase === 'starting' || current.phase === 'stopping') {
    // An editorial refresh may remove the transition being played. Do not
    // strand the device on an empty, non-looping phase with no ended event.
    return library?.[current.phase]?.length
      ? current
      : advanceDreamcatcherPlayback(current, working, library)
  }
