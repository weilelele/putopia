...
export function UpdateTimeline({ updates }: { updates: DashboardUpdate[] }) {
  return <ol className="update-timeline">{updates.map(item => <li key={item.id} className="update-row">
    ...
    <Link className={`update-content${item.image ? ' update-content--media' : ''}`} href={item.href} scroll={false}>
      {item.image && <SmartImage src={item.image} alt="" width={120} height={80} sizes="120px" className="update-image" />}
...
export function EventRail({ events }: { events: DashboardEvent[] }) {
  ...
  const move = (direction: number) => {
    ...
    if (el && card) { el.scrollTo({ left: card.offsetLeft - (el.firstElementChild as HTMLElement).offsetLeft, behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' }); setPosition(next) }
  }
