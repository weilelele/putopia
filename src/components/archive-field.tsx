import { ArchiveInput, ArchiveSelect, ArchiveTextarea } from './archive-input'
...
export function ArchiveField({ children, className = '', error, helpText, htmlFor, label }: ArchiveFieldProps) {
  ...
  const controls = Children.map(children, child => {
    if (!isValidElement(child) || !(['input', 'textarea', 'select', ArchiveInput, ArchiveTextarea, ArchiveSelect] as unknown[]).includes(child.type)) return child
