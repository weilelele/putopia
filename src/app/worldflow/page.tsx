...
export default async function WorldflowPage({ searchParams }: { searchParams: Promise<{ world?: string }> }) {
  ...
  const [{ data: profile }, { data: worlds }, { data: assets }, { data: feedback }, query] = await Promise.all([
    ...
    admin.from('worldflow_feedback').select('*').is('resolved_at', null).order('created_at', { ascending: false }),
    searchParams,
  ])

  return <WorldflowClient
    assets={(assets ?? []) as WorldflowAsset[]}
    feedback={(feedback ?? []) as WorldflowFeedback[]}
