  ...
  return (
    <div>
      ...
      >
        ...
        {uploading && (
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(7,9,18,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#C84406', fontSize: '12px' }}>
            上传中...
          </div>
        )}
      </div>
      {value && (
        <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
          <button onClick={() => fileRef.current?.click()} style={{ background: 'none', border: '1px solid rgba(227,82,5,0.16)', color: 'rgba(245,245,245,0.35)', cursor: 'pointer', fontFamily: 'monospace', fontSize: 'var(--fs-caption)', padding: '3px 10px' }}>更换图片</button>
...
export default function DevicesAdmin() {
  ...
  const STATUS_COLOR: Record<string, string> = { available: '#20D890', in_use: '#C84406', needs_repair: '#E83030', unknown: 'rgba(245,245,245,0.35)' }
                ...
                <tr key={d.id}>
                  <td style={S.td}>
                    {d.image_path
                      ...
                      : <div style={{ width: '60px', height: '40px', background: '#0F1430', border: '1px solid rgba(227,82,5,0.16)' }} />
                    }
                  </td>
                  <td style={{ ...S.td, color: 'rgba(245,245,245,0.35)', fontSize: 'var(--fs-caption)' }}>{d.id}</td>
                  <td style={{ ...S.td, color: '#F5F5F5' }}>{d.name}</td>
                  <td style={S.td}>
                    <span style={{ fontSize: 'var(--fs-caption)', color: d.knowledge === 'known' ? '#20D890' : '#C84406' }}>
                  ...
                  <td style={S.td}>
                    ...
                    {d.knowledge === 'unknown' && (
                      <span style={{ fontSize: 'var(--fs-caption)', color: '#C84406' }}>{d.exploration_progress}%</span>
                    )}
                  </td>
                  <td style={{ ...S.td, whiteSpace: 'nowrap' }}>
                    <button onClick={() => openEdit(d)} style={{ marginRight: '8px', background: 'none', border: 'none', color: '#C84406', cursor: 'pointer', fontFamily: 'monospace', fontSize: '12px' }}>编辑</button>
      ...
      {showForm && (
        <div ref={formRef} style={{ ...S.card, border: '1px solid #C84406' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <div style={{ color: '#C84406', fontSize: '12px', letterSpacing: '0.2em' }}>{editId ? `编辑: ${editId}` : '新增设备'}</div>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <input
                  ...
                  style={{ flex: 1 }}
                />
                <span style={{ color: '#C84406', fontSize: '13px', minWidth: '36px' }}>{form.exploration_progress}%</span>
              </div>
            </div>
          )}
