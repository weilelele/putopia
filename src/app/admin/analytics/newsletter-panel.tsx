  ...
  return (
    <div style={{ flex: 1, minWidth: 280, background: CARD_BG, border: `1px solid ${BORDER}`, borderTop: `2px solid ${accent}`, padding: '1.1rem 1.25rem', borderRadius: 2 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.18em', color: accent, fontWeight: 700 }}>{title}</div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>clicked {base.toLocaleString()}</div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.55rem' }}>
        {stages.map((s, i) => {
          ...
          return (
            <div key={s.label}>
              {i > 0 && (
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, paddingLeft: '0.25rem', marginBottom: '0.2rem' }}>↓ {stepRate}</div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.08em', color: DIM, width: 96, flexShrink: 0 }}>{s.label}</div>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: STAR, width: 40, textAlign: 'right', flexShrink: 0 }}>{s.value.toLocaleString()}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, width: 32, textAlign: 'right', flexShrink: 0 }}>{base > 0 ? pct(s.value, base) : '—'}</div>
              </div>
              {s.note && (
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, paddingLeft: '102px', marginTop: '0.15rem' }}>{s.note}</div>
              )}
            </div>
          )
        })}
...
export function NewsletterPanel({ data }: { data: NewsletterFunnel }) {
  ...
  return (
    <div style={{ marginTop: '2rem' }}>
      ...
      <div style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap', background: CARD_BG, border: `1px solid ${BORDER}`, padding: '1.1rem 1.25rem', borderRadius: 2, marginBottom: '1rem' }}>
        {kpis.map(k => (
          <div key={k.label} style={{ minWidth: 96 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginBottom: '0.3rem' }}>{k.label}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 22, color: k.color, fontWeight: 700, lineHeight: 1 }}>{k.val}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginTop: '0.3rem' }}>{k.sub}</div>
          </div>
        ))}
      </div>

      {!data.tracked && (
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: AMBER, background: 'rgba(232,160,32,0.06)', border: '1px solid rgba(232,160,32,0.2)', padding: '0.6rem 0.9rem', borderRadius: 2, marginBottom: '1rem' }}>
      ...
      <div style={{ marginTop: '1.25rem', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, lineHeight: 1.9 }}>
