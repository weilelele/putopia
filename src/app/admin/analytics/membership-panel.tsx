  ...
  return (
    <div style={{ flex: 1, minWidth: 280, background: CARD_BG, border: `1px solid ${BORDER}`, borderTop: `2px solid ${accent}`, padding: '1.1rem 1.25rem', borderRadius: 2 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <div style={{ fontFamily: 'monospace', fontSize: 11, letterSpacing: '0.18em', color: accent, fontWeight: 700 }}>
          {title}
        </div>
        <div style={{ fontFamily: 'monospace', fontSize: 9, color: MUTED }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.55rem' }}>
        {stages.map((s, i) => {
          ...
          return (
            <div key={s.label}>
              {i > 0 && (
                <div style={{ fontFamily: 'monospace', fontSize: 8, color: MUTED, paddingLeft: '0.25rem', marginBottom: '0.2rem' }}>
                  ↓ {stepRate}
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <div style={{ fontFamily: 'monospace', fontSize: 9, letterSpacing: '0.08em', color: DIM, width: 96, flexShrink: 0 }}>
                  {s.label}
                </div>
                ...
                <div style={{ fontFamily: 'monospace', fontSize: 11, color: STAR, width: 40, textAlign: 'right', flexShrink: 0 }}>
                  {s.value.toLocaleString()}
                </div>
                <div style={{ fontFamily: 'monospace', fontSize: 8, color: MUTED, width: 32, textAlign: 'right', flexShrink: 0 }}>
                  {cohort > 0 ? pct(s.value, cohort) : '—'}
                </div>
              </div>
              {s.note && (
                <div style={{ fontFamily: 'monospace', fontSize: 8, color: MUTED, paddingLeft: '102px', marginTop: '0.15rem' }}>
                  {s.note}
                </div>
              )}
            </div>
