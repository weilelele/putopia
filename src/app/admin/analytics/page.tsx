...
function TrafficRef({ count, count30d, hide30d = false }: { count: number; count30d: number; hide30d?: boolean }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '0.75rem 1rem', background: '#070c1a', border: `1px solid ${BORDER}`, marginBottom: '0.5rem' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.2em', color: MUTED }}>
        HOMEPAGE TRAFFIC (去重浏览器)
      </div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: DIM }}>
        {count.toLocaleString()} <span style={{ fontSize: 'var(--fs-caption)', color: MUTED }}>{hide30d ? '当日' : 'all-time'}</span>
      </div>
      {!hide30d && (
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>
          {count30d.toLocaleString()} <span style={{ fontSize: 'var(--fs-caption)' }}>30d</span>
        </div>
      )}
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginLeft: 'auto' }}>
        ↓ 不计入漏斗转化率
      </div>
    </div>
  )
...
function StepRow({ step, prev, maxCount, color = ACCENT, hide30d = false }: {
  ...
  return (
    <div>
      {prev && (
        <div style={{ paddingLeft: '0.5rem', marginBottom: '0.25rem', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: stepRate ? MUTED : '#151B3A' }}>
          {stepRate ? `↓ ${stepRate}` : '↓'}
        </div>
      )}
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.12em', color: DIM, width: 180, flexShrink: 0 }}>
          {step.step_label.toUpperCase()}
        </div>
        <FunnelBar count={step.count_all_time} max={maxCount} color={color} />
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: STAR, width: 48, textAlign: 'right', flexShrink: 0 }}>
          {step.count_all_time.toLocaleString()}
        </div>
        {!hide30d && (
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, width: 52, textAlign: 'right', flexShrink: 0 }}>
            30d: {step.count_30d}
          </div>
        )}
      </div>
...
function CoreRatesCard({ started, email, registered, daily = false }: {
  ...
  const labelStyle = { fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginBottom: '0.25rem' } as const
  const numStyle   = { fontFamily: 'var(--font-mono)', fontSize: 22, color: ACCENT, fontWeight: 700, lineHeight: 1 } as const
  const denStyle   = { fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginTop: '0.25rem' } as const
  ...
  return (
    <div style={{ marginTop: '1.5rem', padding: '1rem 1.25rem', background: 'rgba(200,68,6,0.03)', border: '1px solid rgba(200,68,6,0.15)', borderRadius: 2 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: '#6E6B5E', marginBottom: '0.75rem' }}>
      ...
      {daily && (
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginTop: '0.75rem', lineHeight: 1.7 }}>
...
function BeforeAfterComparison({ steps }: { steps: SnapshotRow[] }) {
  ...
  return (
    <div style={{ marginTop: '1.5rem', padding: '1rem 1.25rem', background: 'rgba(255,90,31,0.03)', border: `1px solid rgba(255,90,31,0.18)`, borderRadius: 2 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: MUTED, marginBottom: '1rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 52px 1fr 52px', gap: '0.5rem', alignItems: 'center', marginBottom: '0.5rem' }}>
        <div />
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, letterSpacing: '0.15em' }}>BEFORE</div>
        <div />
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: ACCENT, letterSpacing: '0.15em' }}>AFTER</div>
        <div />
      </div>
            ...
            <div key={row.label}>
              {i > 0 && (preRate || postRate) && (
                <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 52px 1fr 52px', gap: '0.5rem', marginBottom: '0.2rem' }}>
                  <div />
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, paddingLeft: '0.25rem' }}>{preRate ?? ''}</div>
                  <div />
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: ACCENT, opacity: 0.7, paddingLeft: '0.25rem' }}>{postRate ?? ''}</div>
                  <div />
                </div>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 52px 1fr 52px', gap: '0.5rem', alignItems: 'center' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.08em', color: row.postOnly ? MUTED : DIM }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: row.postOnly ? MUTED : STAR, textAlign: 'right' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: STAR, textAlign: 'right' }}>
      ...
      <div style={{ marginTop: '0.75rem', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>
  ...
  return (
    <div style={{ background: CARD_BG, border: `1px solid ${isLatest ? ACCENT : BORDER}`, padding: '1.25rem 1.5rem', borderRadius: 2 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: isLatest ? ACCENT : MUTED }}>
          {new Date(run.captured_at).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}
        </div>
        {run.version_tag && (
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: OK_COLOR, background: 'rgba(32,216,144,0.1)', border: '1px solid rgba(32,216,144,0.25)', padding: '2px 8px' }}>
            {run.version_tag}
          </div>
        )}
        {isLatest && (
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: ACCENT, background: 'rgba(200,68,6,0.08)', border: '1px solid rgba(200,68,6,0.25)', padding: '2px 8px' }}>
            LATEST
          </div>
        )}
      </div>
      ...
      {adRef && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '0.75rem 1rem', background: '#070c1a', border: `1px solid ${BORDER}`, marginBottom: '0.5rem' }}>
          ...
          {phSteps[0]?.count_all_time > 0 && (
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: ACCENT, marginLeft: 'auto' }}>
      ...
      <div style={{ marginTop: '0.75rem' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: '#2A3A5A', marginBottom: '0.5rem' }}>
      ...
      <div style={{ margin: '1rem 0', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: '#2A3A5A', letterSpacing: '0.2em' }}>SUPABASE · 完整历史数据</div>
        <div style={{ flex: 1, height: 1, background: '#1A2438' }} />
      </div>
      ...
      <div style={{ marginTop: '1rem', paddingTop: '0.75rem', borderTop: `1px solid ${BORDER}`, display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
        {phSteps.length >= 2 && (
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>Started→末题完成</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: ACCENT, fontWeight: 700 }}>
          ...
          return (
            <>
              {emailStep && clickedStep && (
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>Email→链接点击</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#E8A020', fontWeight: 700 }}>
              ...
              {clickedStep && regStep && (
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>点击→注册完成</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#E8A020', fontWeight: 700 }}>
              ...
              {emailStep && regStep && (
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>Email→注册总转化</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#E8A020', fontWeight: 700 }}>
  ...
  return (
    <div style={{ background: CARD_BG, border: `1px solid ${ACCENT}`, padding: '1.25rem 1.5rem', borderRadius: 2, marginBottom: '2rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: ACCENT }}>
          单日漏斗 · DAILY FUNNEL
        </div>
      ...
      <div style={{ margin: '1rem 0', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <div style={{ flex: 1, height: 1, background: '#1A2438' }} />
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: '#2A3A5A', letterSpacing: '0.2em' }}>SUPABASE · 当日</div>
        <div style={{ flex: 1, height: 1, background: '#1A2438' }} />
      </div>
      ...
      <div style={{ marginTop: '1rem', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, lineHeight: 1.8 }}>
...
function EmailCaptureComparison({ rows }: { rows: VersionCapture[] }) {
  ...
  const col = (tag: string, tagColor: string, cap: VersionCapture | undefined) => {
    ...
    return (
      <div style={{ flex: 1, minWidth: 150 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.15em', color: tagColor, marginBottom: '0.4rem' }}>{tag}</div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 30, color: rate != null ? STAR : MUTED, fontWeight: 700, lineHeight: 1 }}>
          {rate != null ? `${rate.toFixed(1)}%` : '—'}
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginTop: '0.4rem' }}>
  ...
  return (
    <div style={{ background: CARD_BG, border: `1px solid ${ACCENT}`, padding: '1.25rem 1.5rem', borderRadius: 2, marginBottom: '2rem' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: ACCENT, marginBottom: '0.35rem' }}>
        ONBOARDING 改版前后 · 邮箱留资率 (v2 → v3)
      </div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginBottom: '1.1rem', lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 20, color: MUTED, alignSelf: 'center' }}>→</div>
        {col('AFTER · v3', ACCENT, v3)}
        <div style={{ borderLeft: `1px solid ${BORDER}`, paddingLeft: '1.5rem', alignSelf: 'center' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginBottom: '0.4rem' }}>变化 (pp)</div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 30, fontWeight: 700, lineHeight: 1, color: delta == null ? MUTED : delta >= 0 ? OK_COLOR : '#D8203A' }}>
      ...
      {thin && (
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: '#E8A020', marginTop: '0.9rem', lineHeight: 1.7 }}>
...
function HistoryTable({ runs }: { runs: Run[] }) {
  ...
  return (
    <div style={{ marginTop: '2rem' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.3em', color: MUTED, marginBottom: '0.75rem' }}>
        HISTORY — overall CVR (Onboarding Started → Email)
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)' }}>
          <thead>
            <tr>
...
function RecoveryCohortPanel({ rows }: { rows: RecoveryRow[] }) {
  ...
  return (
    <div style={{ marginTop: '2rem', background: CARD_BG, border: `1px solid ${ACCENT}`, padding: '1.25rem 1.5rem', borderRadius: 2 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.25em', color: ACCENT, marginBottom: '0.4rem' }}>
        INVITE RECOVERY · 失败补发追溯
      </div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginBottom: '1rem' }}>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
        {[
          ...
          <div key={s.label}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginBottom: '0.25rem' }}>{s.label}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 22, color: s.color, fontWeight: 700, lineHeight: 1 }}>
...
function DailyLoginsPanel({ rows }: { rows: DailyRow[] }) {
  ...
  return (
    <div style={{ marginTop: '2rem', background: CARD_BG, border: `1px solid ${ACCENT}`, padding: '1.25rem 1.5rem', borderRadius: 2 }}>
      ...
      {!hasData ? (
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, padding: '0.5rem 0' }}>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '1.25rem', marginBottom: '0.75rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: DIM }}>新增</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <span style={{ width: 10, height: 10, background: RET_COLOR, display: 'inline-block', borderRadius: 1 }} />
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: DIM }}>回流</span>
            </div>
            <div style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            {chartRows.map((r, i) => (
              <div key={r.day} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: i === 0 ? STAR : DIM, width: 44, flexShrink: 0 }}>
                  {fmtDay(r.day)}
                </div>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: NEW_COLOR, width: 32, textAlign: 'right', flexShrink: 0 }}>
                  {r.newUsers}
                </div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: RET_COLOR, width: 32, textAlign: 'right', flexShrink: 0 }}>
                  {r.returningUsers}
                </div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: STAR, width: 36, textAlign: 'right', flexShrink: 0 }}>
                  {r.activeUsers}
                </div>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.6rem', justifyContent: 'flex-end' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: NEW_COLOR, width: 32, textAlign: 'right' }}>新</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: RET_COLOR, width: 32, textAlign: 'right' }}>回流</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: STAR, width: 36, textAlign: 'right' }}>活跃</div>
          </div>
        </>
      )}
    </div>
    ...
    <>
      ...
      {runs.length === 0 ? (
        <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, padding: '2rem', textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: DIM, lineHeight: 2 }}>
      ...
      <div style={{ marginTop: '2rem', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, lineHeight: 2 }}>
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', letterSpacing: '0.3em', color: MUTED, marginBottom: '0.5rem' }}>
            FUNNEL & RETENTION
          </div>
          <h1 style={{ fontFamily: 'var(--font-mono)', fontSize: 18, letterSpacing: '0.15em', color: STAR, margin: 0 }}>
            ANALYTICS
          </h1>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-caption)', color: MUTED, marginTop: '0.4rem' }}>
