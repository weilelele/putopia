  ...
  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        <div style={{ color: 'rgba(245,245,245,0.35)', fontSize: 'var(--fs-caption)', letterSpacing: '0.3em' }}>ADMIN // VOYAGERS</div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
          {summary.map(([label, count]) => (
            <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '4px 10px', background: '#151B3A', border: '1px solid rgba(255,107,53,0.16)', fontSize: '12px', color: 'rgba(245,245,245,0.55)', fontFamily: 'var(--font-mono)' }}>
              {label}
              <span style={{ padding: '0 6px', borderRadius: '999px', background: 'rgba(232,93,4,0.14)', color: '#FF8A5C', fontSize: 'var(--fs-caption)' }}>{count}</span>
            </span>
          ))}
        </div>
      )}
