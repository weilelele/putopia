...
<h2>NAV-01 · Primary tabs</h2>
<p>Dashboard / Intel / Devices / Worlds / Voyagers. Select a preview below.</p>
...
<h2>New dispatch from the Collective</h2>
<PublisherIdentity name="Mira · fixture" />
<p className="intel-summary">Read the full dispatch in its detail view.</p>
<NewsMedia images={['/assets/ui-kit/world-records-antenna.png','/assets/ui-kit/signal-check-mineral-plain.png']} title="Dispatch fixture" />
<ArchiveButton variant="primary" fullWidth onClick={() => setSheet(true)}>READ INTEL →</ArchiveButton>
</article>
<h3 className="archive-list-heading">RECENT</h3>
<article className="intel-recent-story intel-recent-story--media">
<a className="intel-recent-row" href="#form-01">
<div>
<span className="intel-entry-meta">NOTICE · 08 SEP 2026</span>
<h3>A signal has been recorded</h3>
</div>
</a>
<PublisherIdentity name="Mira · fixture" />
<a className="intel-story-media" href="#form-01">
<NewsMedia images={['/assets/ui-kit/signal-check-mineral-plain.png']} title="Recent fixture" />
</a>
</article>
</> : view === 'devices' ? <ArchiveCard>
<h3>Console · fixture</h3>
<p>Camera offline</p>
<p>Connection status is separate from shipment progress.</p>
<ol>
<li>Confirmed</li>
<li>Preparing · current</li>
<li>Shipping</li>
<li>Delivered</li>
</ol>
...
<span className="voyager-directory-portrait">MI</span>
<div>
<h3>Mira · fixture</h3>
<p>VOYAGER</p>
<p>128 observation days</p>
</div>
</div>
<details className="voyager-directory-details">
<summary>Worlds &amp; observations</summary>
<p>2 worlds · profile details remain available.</p>
</details>
</article>
</> : view === 'profile' ? <ArchiveField htmlFor="kit-profile-name" label="DISPLAY NAME">
<ArchiveInput id="kit-profile-name" value={name} onChange={event => setName(event.target.value)} />
</ArchiveField> : <ArchiveCard>
<h3>Voyager Log · fixture</h3>
<p>Secondary pages use a back control and title, with no brand header.</p>
</ArchiveCard>}
...
<ArchiveButton onClick={() => setButtonBusy(value => !value)}>Toggle loading example</ArchiveButton>
<ArchiveButton loading={buttonBusy}>Primary action</ArchiveButton>
<ArchiveButton variant="secondary">Secondary action</ArchiveButton>
<ArchiveButton variant="ghost">Tertiary action</ArchiveButton>
<ArchiveButton variant="destructive" onClick={() => setConfirmDelete(true)}>Delete sample record</ArchiveButton>
<ArchiveButton disabled>Disabled action</ArchiveButton>
<ArchiveButton size="compact" variant="secondary">Compact · 44px target</ArchiveButton>
</div>
</section>
      <section className="uk-section" id="nav-02">
<h2>NAV-02 · Return context</h2>
<p>Open Logs from the bottom of Voyagers, then return to the same list.</p>
<Link href="/logs">Open real Voyager Logs</Link>
</section>
      <ArchiveSheet open={confirmDelete} onClose={() => setConfirmDelete(false)} title="Delete sample record?">
<p>This affects only this local example.</p>
<ArchiveButton variant="secondary" onClick={() => setConfirmDelete(false)}>Cancel</ArchiveButton>
<ArchiveButton variant="destructive" onClick={() => { setDeleted(true); setConfirmDelete(false) }}>Delete sample record</ArchiveButton>
...
<ArchiveInput id="kit-sheet-name" value={draftName} onChange={event => setDraftName(event.target.value)} />
      ...
      <section className="uk-section" id="form-01">
<h2>FORM-01 · Validation</h2>
<ArchiveField htmlFor="kit-name" label="DISPLAY NAME" helpText="Required. Visible to the Collective." error={error}>
<ArchiveInput id="kit-name" value={name} onChange={event => { setName(event.target.value); setError(''); setResult('ready') }} />
</ArchiveField>
<ArchiveButton onClick={() => { if (!name.trim()) { setError('Enter a display name.'); document.getElementById('kit-name')?.focus() } else setError('') }}>Validate</ArchiveButton>
</section>
      <section className="uk-section" id="form-controls">
<h2>FORM-02 · Native controls</h2>
<p>These are the same fields used by product forms. Values stay in this preview.</p>
        <ArchiveField htmlFor="kit-email" label="EMAIL">
<ArchiveInput id="kit-email" type="email" placeholder="name@example.com" />
</ArchiveField>
        <ArchiveField htmlFor="kit-note" label="NOTE">
<ArchiveTextarea id="kit-note" rows={3} placeholder="Write a note" />
</ArchiveField>
        <ArchiveField htmlFor="kit-category" label="CATEGORY">
<ArchiveSelect id="kit-category" defaultValue="signal">
<option value="signal">Signal</option>
<option value="world">World</option>
</ArchiveSelect>
</ArchiveField>
        <label>
<ArchiveInput type="checkbox" defaultChecked /> Include this item</label>
        <fieldset>
<legend>Visibility</legend>
<label>
<ArchiveInput type="radio" name="kit-visibility" value="public" defaultChecked /> Public</label>
<label>
<ArchiveInput type="radio" name="kit-visibility" value="private" /> Private</label>
</fieldset>
        <ArchiveField htmlFor="kit-range" label="VOLUME">
<ArchiveInput id="kit-range" type="range" min={0} max={100} defaultValue={50} />
</ArchiveField>
        <ArchiveField htmlFor="kit-file" label="LOCAL FILE">
<ArchiveInput id="kit-file" type="file" />
</ArchiveField>
        <ArchiveField htmlFor="kit-disabled" label="READ ONLY">
<ArchiveInput id="kit-disabled" disabled value="Unavailable in this state" />
</ArchiveField>
      </section>
      <section className="uk-section" id="submit-01">
<h2>SUBMIT-01 · Result certainty</h2>
<ArchiveTabs mode="filter" ariaLabel="Submission scenario" activeId={scenario} items={[{id:'success',label:'SUCCESS'},{id:'unknown',label:'UNKNOWN'}]} onChange={setScenario} />
...
<div className="uk-state" role="status" aria-busy={live === 'loading'}>{({loading:'Loading the latest signal…',empty:'No signals have been published yet.',error:'This feed could not be loaded.',offline:'Offline. Saved content is read only; reconnect to participate.',ready:'The latest signal is ready.'})[live]}</div>{['error','offline'].includes(live) && <ArchiveButton variant="secondary" onClick={() => setLive('ready')}>Retry fixture</ArchiveButton>}</section>
      <section className="uk-section uk-span">
<h2>Launch · Playback + 3 seconds</h2>
<p>The protected flip wordmark plays at entry. Tap anywhere to enter Dashboard immediately, or stay for 3 seconds after playback finishes. Reduced motion shows the original static wordmark.</p>
<Link href="/welcome">Preview startup screen</Link>
</section>
    </div>
  </main>
}
