...
所有类型统一为日期轨道、左侧缩略图、右侧信息。手机缩略图宽 88px（窄屏 72px），桌面 120px，比例 5:4。右侧依次显示类型、最多两行标题、紧邻昵称的作者头像、最多一行摘要。多图只展示首图并标注剩余数量，完整图片在详情查看；无图用类型图标占位，机密条目只显示锁图标及访问提示。字号不低于 12px，不改变内容池、排序、时效与权限规则。
