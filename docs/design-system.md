...
**一级品牌区实施默认值：**左右 16px、顶部/底部各 8px，图形宽 40px、字标宽 160px、二者间距 12px，高度按原图比例自然计算，内容区最小 64px（不含顶部安全区）。不与页面操作争同一行，不加专属分隔线；一级页不再显示重复 Tab 名称，也不保留标题行的高度或留白；页面名称保留为仅辅助技术可见的 h1。有必要动作时呈现紧凑操作行，无动作时直接进入正文。一级品牌区不套用下述 56px 紧凑标题栏高度。Logo 默认静态、非链接，避免无意切走当前 Tab；入口品牌动画按减少动态偏好降级。
