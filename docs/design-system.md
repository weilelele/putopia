...
**应用内页面不重复展示顶部图形 Logo 或文字 Logo，包括五个一级 Tab、二级详情、申请、登录与注册页。** Voyager 身份状态卡中的原设备符号作为内容保留。 原始品牌资产与主配色不变，启动动画、明确宣传/品牌展示场景仍可使用。此前“一级页统一保留品牌区”的默认值已被用户最新指示取代。桌面/横屏的单一侧栏品牌区域恢复，它不属于每个内容页重复的页头。
...
- Devices 主导航图标使用原 `vi-icon.png`，保持原比例和橙色，不重绘或染色；激活状态依靠导航标记与文字区分。Web、UI Kit、原生离线导航一致。
- 登录后的欢迎标题统一为 WELCOME, BROKER。实际 Applicant / Voyager / Architect 身份只在角色标签显示，游客仍显示统计。
