...
Devices 导航已用原品牌图形替换手柄；欢迎标题统一 WELCOME, BROKER；Intel 保留首条宽图，后续条目使用右侧缩略图。Devices 城市标签居左。Voyagers 批次箭头使用共享 BatchTabs，首尾禁用且不覆盖标签。Worlds Live 新增独立加载占位，避免 WORLD RECORDS 闪屏。UI Kit 与原生离线展示同步；iOS 仍需原生视觉验收。
