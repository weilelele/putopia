...
> **2026-09-10 最新展示规则：游客与登录态头部互斥。** 游客显示原三个数字指标，登录后统一显示 WELCOME, VOYAGER 欢迎与个人身份状态（实际身份在角色标签显示）；两块不叠加。五个一级 Tab 在移动端恢复紧凑原图形＋文字 Logo；二级及更深页面无 Logo；桌面/横屏统一侧栏保留品牌，内容区不重复。Intel 列表、新闻详情及 Updates 中的新闻条目保留作者头像（紧邻昵称）和图片。UpdateTimeline 与 EventRail 必须执行已确认的内容池、上限、去重和完成态规则。
...
评论中的头像和姓名共用一个可聚焦的详情入口，打开该作者的 MemberProfileSheet（ArchiveSheet），不跳转到当前用户资料。使用作者 profile ID 获取公开资料；缺失身份时不伪造入口。弹窗支持加载、失败重试及资料不可用状态；关闭后焦点与阅读位置返回原评论。

### C17 · CommentThread / Composer

评论采用平面轻列表与细分隔线，不给每条评论套切角卡片；回复与删除采用至少 44px 点击区的轻量文字动作。
