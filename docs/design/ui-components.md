...
- 一级：使用 RootBrandHeader，图形宽 28px、字标宽 108–120px，与必要动作共用最小 56px 首行；页面 h1 仅辅助技术可见。游客 Dashboard 改用 DashboardGuestHeader：原字标、原图形、原介绍语、设备原图、View Device、两项 CTA 与原统计；不额外挂 RootBrandHeader。
- 原图等比 contain、不重画、不滤镜；长字/窄屏不拉伸图片。
- 一级 Logo 链接到 Dashboard 且不吸顶；明确品牌展示中的链接按实际目标命名，不能统一叫返回 Home。
