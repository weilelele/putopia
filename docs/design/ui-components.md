...
| Dashboard 缩略图 | 390px 下 120×80，3:2，固定左图右文 |
...
规则：取最近最多 10 条可见动态；有 10 条就显示 10 条，少于 10 条不伪造。按 occurredAt 降序；同时间用稳定 id 排序。过滤权限后再取有效最近 10 条。最新消息不得自动插入使阅读位置跳动；可显示「有新动态」刷新提示。
...
- 图片 120×80，左图右文；分类 12px、标题目标 14/700、说明 13px，图文间 12px。标题最多建议两行，但必要名称可增长。
- 无图行移除图片列，正文占时间列右侧全部宽度，不留空框。无图行按内容高度自然排布。
- <360px 可将时间列改 32px、首间距 8px、缩略图 96×64，保留图文间 12px。200% 字体放大时允许时间移至行上方，确保阅读，不缩字。
...
目标数据合同：id、title、简短目的、可选 image、availability、真实 action(label/href 或 handler)、可选真实截止时间/资格原因。活动顺序由业务优先级决定，不套用 Updates 时间倒序。
...
- `LaunchScreen` 是 iOS 离线启动层；Web `/welcome` 使用原 FlipWordmark。进入时播放一次，点击或 3 秒结束，跳转只发生一次。
