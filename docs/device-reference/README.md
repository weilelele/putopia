...
**设计与图片生成第一原则：照片级真实感。** 所有效果图、场景图与宣传帧以真实相机拍摄般的可信度为第一优先级；有明显 AI 痕迹或渲染感的图必须返工或替换，不能列为正式可用素材。原始造型图与 UI 母版保留其结构／界面参考用途。详见[准入检查与生成规范](image-generation-spec.zh.md)。
