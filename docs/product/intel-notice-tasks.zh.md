...
到期保留历史文章、图片和评论，不删除或隐藏。列表和详情标记状态；页面打开期间到达截止时间会自动更新。排序仍按 timestamp 倒序，截止时间不影响排序。阅读时按浏览者设备时区自动换算，显示截止日期对应的 UTC 偏移（如 UTC+08:00），不显示地区标识；偏移包含该日期的夏令时变化。设置未来 timestamp 不等于定时隐藏发布：文章仍可阅读，只是不算进行中任务。
